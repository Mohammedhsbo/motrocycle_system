...
export default function TransactionHistory({ lang }: { lang: 'en' | 'ar' }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!query.isLoading && !query.isError && result && result.items.length > 0 && <><DataList className="customer-list">{result.items.map((order) => <button className="customer-row" key={order.id} onClick={() => navigate(`/orders/${order.id}`)}><div className="customer-main"><strong>{order.orderNumber}</strong><span>{order.customer.name} · {order.customer.phone}</span><span>{order.itemCount} {isRtl ? 'عناصر' : 'items'} · {new Date(order.createdAt).toLocaleString(isRtl ? 'ar-EG' : 'en-EG')}</span></div><div className="customer-stats"><span className="status-pill">{order.status}</span><strong>{order.netAmount.toLocaleString()} {isRtl ? 'ج.م' : 'EGP'}</strong></div></button>)}</DataList><div className="panel-heading"><span>{result.total} {isRtl ? 'معاملة' : 'transactions'}</span><div><button className="icon-button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)} title="Previous">{isRtl ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}</button><span>{page} / {totalPages}</span><button className="icon-button" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)} title="Next">{isRtl ? <ChevronLeft size={18} /> : <ChevronRight size={18} />}</button></div></div></>}
    </div>
  );
}

...
export default function ActiveReservations({ lang, branchId }: { lang: 'en' | 'ar'; branchId?: string }) {
  ...
  return <section className="desktop-page" dir={isRtl ? 'rtl' : 'ltr'}>
    ...
    {reservations.isLoading && <div className="state-panel">{isRtl ? 'جاري التحميل...' : 'Loading reservations...'}</div>}
    {reservations.isError && <div className="state-panel" role="alert"><p>{isRtl ? 'تعذر تحميل الحجوزات.' : 'Could not load active reservations.'}</p><button className="secondary-action" onClick={() => reservations.refetch()}>{isRtl ? 'إعادة المحاولة' : 'Retry'}</button></div>}
    {!reservations.isLoading && !reservations.isError && filteredReservations.length === 0 && <div className="state-panel"><Bookmark size={28} /><p>{isRtl ? 'لا توجد حجوزات نشطة.' : 'No active reservations.'}</p></div>}
    {!reservations.isLoading && !reservations.isError && <div className="customer-list">{filteredReservations.map((reservation) => <button className="customer-row" key={reservation.id} onClick={() => navigate(`/reservations/${reservation.id}`)}><div className="customer-avatar"><Bookmark size={18} /></div><div className="customer-main"><strong>{reservation.reservationNumber}</strong><span>{reservation.customer.name} · {reservation.customer.phone}</span><span>{reservation.motorcycle.vin} · {reservation.motorcycle.model}</span></div><div className="customer-stats"><span>{isRtl ? 'المتبقي' : 'Remaining'}: {reservation.remainingAmount.toLocaleString()}</span><span>{new Date(reservation.expiresAt).toLocaleDateString(isRtl ? 'ar-EG' : 'en-EG')}</span></div></button>)}</div>}
  </section>;
}
...
export default function Installments({ lang }: { lang: Lang }) {
  ...
  const money = (value: number) => `${Number(value || 0).toLocaleString(isRtl ? 'ar-EG' : 'en-US', { maximumFractionDigits: 2 })} ${isRtl ? 'ج.م' : 'EGP'}`;
  ...
  return <section className="desktop-page" dir={isRtl ? 'rtl' : 'ltr'}>
    ...
    {isLoading && <div className="state-panel">{isRtl ? 'جاري تحميل العقود...' : 'Loading contracts...'}</div>}
    {isError && <div className="state-panel">{isRtl ? 'تعذر تحميل عقود التمويل.' : 'Could not load financing contracts.'}</div>}
    {!isLoading && !isError && contracts.length === 0 && <div className="state-panel">{isRtl ? 'لا توجد عقود تمويل.' : 'No financing contracts found.'}</div>}
    {!isLoading && !isError && contracts.length > 0 && <div className="contract-table"><div className="contract-header"><span>{isRtl ? 'العقد' : 'Contract'}</span><span>{isRtl ? 'العميل' : 'Customer'}</span><span>{isRtl ? 'المبلغ' : 'Amount'}</span><span>{isRtl ? 'الحالة' : 'Status'}</span></div>{contracts.map(contract => <button className="contract-row" key={contract.id} onClick={() => setSelected(contract)}><span><strong>{contract.contractNumber}</strong><small>{contract.numberOfInstallments} {isRtl ? 'قسط' : 'installments'}</small></span><span>{contract.customer?.name || contract.customerId}</span><span>{money(contract.totalAmount)}</span><span className={`status-text status-${contract.status}`}>{contract.status}</span></button>)}</div>}
